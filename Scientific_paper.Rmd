...
zijn tussen alle de analisten samen(p-waarde = 0,07672, alfa = ,05). Tussen
de analisten 9308 en 8705 was wel een significant verschil (p-waarde =
0,03118, alfa = ,05). Dit zie je ook aan de flinke verschillen tussen de
...
`chi-square` test op los gelaten. Hier is de p-waarde 0,008905(alfa = ,05).
...
```{r, original-file-to-final-version, eval=FALSE}
...
```{r, shorten-to-1-identifier, eval=FALSE}
...
```{r, hanze-pie-chart, fig.cap='Drie taart grafieken die de tellingen tussen de oogkleuren per locatie vergelijken.', out.width="33%", eval=FALSE}
ggplot(data = Hanze, 
...
(p-waarde = 0.08554, alfa = 0,05). Het verschil tussen de bruine en blauwe
...
mannen en vrouwen(bij alfa = 0.05). Daarnaast zijn er geen andere grote
...
De p-waarde uit de `chi-square` test is p=0,08 en is boven de alfa van 0,05. De p-waarde uit deze chi square is hier niet significant. Er is dus geen aantoonbaar verschil in oogkleur tussen mannen en vrouwen. 
...
De p-waarde van de Pearson’s `Chi-square` test is p=0,07672 (met alfa = 0,05) er is dus geen significant verschil tussen de analisten. Echter, er zijn nog meetfouten aanwezig die vermindert kunnen worden. 
